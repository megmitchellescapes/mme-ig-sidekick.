...
<div class="wrap">
  <header class="hero">
    <div class="kicker">Meghan Mitchell Escapes • Version 2.8</div>
...
function suggestedChecklistAccounts(){
  buildToday(false);
  const ids=todayIds();
  let items=ids.map(h=>saved.find(a=>a.handle===h)).filter(Boolean);
  if(items.length<5){
    const used=new Set(items.map(a=>a.handle));
    const extras=[...saved].filter(a=>!used.has(a.handle)).sort((a,b)=>b.score-a.score);
    items.push(...extras.slice(0,5-items.length));
  }
  return items.slice(0,5).map(a=>({handle:a.handle,url:a.url,bucket:a.bucket,reason:a.reason}));
}
...
function renderAll(){renderTabs();renderSaved();renderToday();renderStats();renderTagBank();renderDestinationTabs();renderEngagementTracker();renderEngagementStats();renderDailyChecklist()}
