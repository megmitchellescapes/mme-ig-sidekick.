...
<div class="wrap">
  <header class="hero">
    <div class="kicker">Meghan Mitchell Escapes • Version 2.10.3</div>
...
def extract_comment_details(caption):
    text = clean_text(caption)
    low = text.lower()
    mapping = [
      ("restaurant", ["restaurant","terrace","menu","dinner","lunch","breakfast","chef","dining"]),
      ("hotel", ["hotel","resort","suite","room","property","villa"]),
      ("beach", ["beach","ocean","sea","water","coast","shore"]),
      ("experience", ["tour","experience","excursion","sailing","boat","hike","spa","class"]),
      ("destination", ["travel","visit","destination","city","island","turkey","italy","greece","portugal"])
    ]
    topic = next((label for label,terms in mapping if any(t in low for t in terms)), "post")
    proper = re.findall(r"\b(?:[A-ZÀ-ÖØ-Ý][A-Za-zÀ-ÿ'’.-]+(?:\s+[A-ZÀ-ÖØ-Ý][A-Za-zÀ-ÿ'’.-]+){0,3})\b", text)
    junk = {"During","After","The","This","That","Our","Its","Instagram","Facebook"}
    proper = [p.strip() for p in proper if p.strip() not in junk]
    return {"text": text, "low": low, "topic": topic, "proper": proper[:4]}

def build_caption_aware_comments(caption, goal):
    d = extract_comment_details(caption)
    low, topic, names = d["low"], d["topic"], d["proper"]

    has_terrace = "terrace" in low
    has_open = any(x in low for x in ["open","opens","opening","reopen"])
    has_sold = "sold out" in low
    event_name = "Assumption week" if "assumption" in low else ""

    if has_terrace and has_open:
        occasion = f" for {event_name}" if event_name else ""
        quick = f"Opening the terrace{occasion} is such a good idea. 😍"
        if has_sold:
            quick = f"With the event nights sold out, opening the terrace{occasion} is such a smart addition. 😍"
        convo = f"Is the terrace usually open during the season, or is this a special opening{occasion}?"
        relation = "Love knowing about special openings like this. These are exactly the little details that are so helpful when planning a client's time in a destination."
        return quick, convo, relation

    if topic == "restaurant":
        quick = "This sounds like such a good one to have on the radar."
        convo = "Is this something you recommend reserving ahead, or is it usually possible to stop in?"
        relation = "I love finding dining details like this. They are the kind of thing that can make an itinerary feel much more connected to the destination."
        return quick, convo, relation

    if topic == "hotel":
        return (
          "This is such a great property detail.",
          "Is this available year-round, or only during certain parts of the season?",
          "Details like this are exactly what I want to know when matching a client with the right stay."
        )

    if topic == "experience":
        return (
          "This is the kind of experience that makes a trip memorable.",
          "How far ahead do you usually recommend booking this?",
          "I love finding experiences like this for clients who want more than a standard sightseeing itinerary."
        )

    if topic == "beach":
        return (
          "What a beautiful setting.",
          "What time of year do you most recommend experiencing this area?",
          "I love seeing places where the setting itself becomes part of the travel experience."
        )

    if topic == "destination":
        return (
          "Okay, this is making a very strong case for the travel list.",
          "What is the one thing you would tell a first-time visitor not to miss?",
          "I love finding destination details like this to keep on my radar for future client trips."
        )

    subject = names[0] if names else ""
    quick = f"This update from {subject} caught my attention." if subject else "This caught my attention."
    return (
      quick,
      "What should a first-time visitor know about this?",
      "Love seeing updates like this. Helpful context to keep on my radar when planning future trips."
    )
...
def comment_suggestions():
    ...
    quick, convo, relation = build_caption_aware_comments(caption, goal)

    if goal=="Start a conversation":
        quick = convo
    elif goal=="Potential client":
        relation = "This looks like such a fun part of the trip. I love seeing the details people end up enjoying most."
    elif goal=="Existing follower":
        relation = "Love this update. Always fun seeing what you share next."
