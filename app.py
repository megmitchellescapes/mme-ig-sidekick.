...
.cal-day{min-height:92px;border:1px solid var(--line);border-radius:12px;background:#fff;padding:7px;cursor:pointer;position:relative}
...
.cal-post{margin-top:5px;padding:5px 6px;border-radius:8px;background:#FFF5F1;font-size:.67rem;line-height:1.25;color:var(--ink)}
...
@media(max-width:760px){
  .cal-day{min-height:78px;padding:5px}.cal-post{font-size:.6rem;padding:4px}.calendar-grid{gap:3px}
}
...
<div class="wrap">
  <header class="hero">
    <div class="kicker">Meghan Mitchell Escapes • Version 2.10</div>
  ...
  <section class="collapsible-section" data-collapse-id="instagramData">
    ...
    <div class="card import-drop">
      ...
      <div class="meta">Choose the JSON or HTML files from your Instagram information download. Sidekick reads them in your browser and stores the handle lists on this device.</div>
...
function extractHandlesFromObject(obj,out){
  if(Array.isArray(obj)){obj.forEach(x=>extractHandlesFromObject(x,out));return}
  if(obj&&typeof obj==="object"){
    if(typeof obj.href==="string"&&obj.href.includes("instagram.com/")){
      const m=obj.href.match(/instagram\.com\/([^/?#]+)/i);if(m)out.add(normalizeIGHandle(m[1]));
    }
    if(typeof obj.value==="string"&&/^[A-Za-z0-9._]{1,30}$/.test(obj.value))out.add(normalizeIGHandle(obj.value));
    Object.values(obj).forEach(v=>extractHandlesFromObject(v,out));
  }
}
function extractHandlesFromHTML(text){
  const out=new Set();
  const re=/instagram\.com\/([^/"'?#<\s]+)/gi;let m;
...
async function importInstagramData(){
  ...
  const following=new Set(),followers=new Set();
  for(const file of files){
    const text=await file.text();
    const target=/following/i.test(file.name)?following:/followers?/i.test(file.name)?followers:null;
    if(!target)continue;
    try{
      if(file.name.toLowerCase().endsWith(".json")){
        const obj=JSON.parse(text);extractHandlesFromObject(obj,target);
      }else{
        extractHandlesFromHTML(text).forEach(h=>target.add(h));
      }
    }catch{}
  }
  const old=igRelState();
  const state={
    following:[...new Set([...old.following,...following])].filter(Boolean),
    followers:[...new Set([...old.followers,...followers])].filter(Boolean),
    imported:new Date().toISOString()
  };
  localStorage.setItem(IG_REL_KEY,JSON.stringify(state));
  renderAll();
  renderDiscoveryFromCache();
  showToast(`Imported ${following.size} following and ${followers.size} followers`);
}
